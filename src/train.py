...
def run(results_dir, model_dir, base_dir, train_file_names, eval_file_names, num_epochs, data_type,
        ...
        exported_disc_path=None, use_3dconv=False, smooth_weight=0.1, lrate=1e-4, adv_weight=0.0001, min_distance=3.,
        max_distance=150.):
    ...
    x = RepeatVector(100)(inpt)
    hacked_model = Model(inputs=in_image,outputs=model['out_image'][1])
    x = TimeDistributed(hacked_model,name ='epistemic_monte_carrlo')(x)
    ...
    model = Model(inputs=inpt,outputs=[variance,softmax_mean]) 
...
class PredictiveEntropy(Layer):
    def build(self, input_shape):
        return super(PredictiveEntropy,self).build(input_shape)
