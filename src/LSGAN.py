...
def loss_l2(D_logits_real, D_logits_fake, input, g_output, targets, data_type, gt_mask, smooth_weight, adv_weight,
    ...
    G_loss = l1_loss   + \
              adv_weight * adv_loss  + smooth_weight * grad_loss
...
def build_model(input, targets, data_type='synthetic', gt_mask=None,
                smooth_weight=0.1, adv_weight=0.0001,
                discriminator_ckpt=None, use_multi_scale=False,
                use_3dconv=False, lrate=1e-4):
    model = {}
    with tf.name_scope('generator'):
        g_output = generator(input, use_multi_scale, use_3dconv)#生成器G out = {'output': out, 'half_scale': out2, 'fourth_scale': out3}
    with tf.name_scope('true_discriminator'):
        true_d_output = discriminator(targets)#鉴别器
    if not discriminator_ckpt is None:
        print('loading fake discriminator.')
        disc_in, fake_d_output = reload_exported_disc(discriminator_ckpt)#每个epoch后保存的模型，从硬盘加载假的D
    else:
        ...
        with tf.name_scope('fake_discriminator'):
            ...
            fake_d_output = discriminator(disc_in, reuse=True)#假的鉴别器D
    with tf.name_scope('loss'):#设置损失函数
