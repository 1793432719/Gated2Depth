...
#结果文件目录，模型目录，基本文件夹，文件名，数据类型，是否使用多尺度，输出disc 路径，是否使用3D卷积，是否计算指标，最大距离，最短距离，显示结果"""
def run(results_dir, model_dir, base_dir, file_names, data_type, data_mode, use_multi_scale=False,
    ...
    if data_type == 'real':
        ...
        model = lsgan.build_model(in_image, gt_image, data_type=data_type, gt_mask=gt_mask, smooth_weight=0.1,
